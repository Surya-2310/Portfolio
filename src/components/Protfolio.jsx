...
},
{
  ...
  type: "Frontend",
  detailsLink: "/restaurant-details",
  icon: "🍽️",
},
{
...
<div className="min-h-screen bg-[#080808] text-white overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-14 items-center">
      <div className="text-center lg:text-left">
        ...
        <div className="mt-8 flex flex-wrap justify-center lg:justify-start gap-4">
          ...
          <a href="/Resume.pdf" download="Resume.pdf" className="bg-orange-300 text-black px-7 py-3 rounded-xl font-bold hover:bg-gray-200">
            Download CV
          </a>
        </div>
  ...
  {imageOpen && (
    <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center px-5">
      <button onClick={() => setImageOpen(false)} className="absolute top-6 right-6 bg-orange-500 px-4 py-2 rounded-xl font-bold">
        Close
      </button>

      <img src={image}alt="full profile"
  ...
  <section id="about" className="py-20 px-6">
    <div className="max-w-5xl mx-auto bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12">
      <h2 className="text-4xl font-black text-orange-400 mb-6 text-center">About Me</h2>
      ...
      <div className="grid md:grid-cols-2 gap-6 text-gray-300 leading-8">
        <div className="bg-black/30 p-6 rounded-2xl border border-white/10">
          <h3 className="text-xl font-bold text-orange-300 mb-3">Frontend Developer</h3>
          <p>
        ...
        <div className="bg-black/30 p-6 rounded-2xl border border-white/10">
          <h3 className="text-xl font-bold text-orange-300 mb-3">Zoho Creator Developer</h3>
          <p>
  ...
  <section id="skills" className="py-20 px-6 bg-black/30">
    <h2 className="text-4xl font-black text-center text-orange-400 mb-4">Skills</h2>

    <p className="text-center text-gray-400 mb-8">Click below button to filter skills by category.</p>

    <div className="text-center mb-10">
      <button onClick={() => setSkillModal(true)}className="bg-orange-500 px-8 py-3 rounded-xl font-bold hover:bg-orange-400">Open Skill Filter</button>
    </div>
    ...
    <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center px-5">
      <div className="w-full max-w-3xl bg-[#111] border border-orange-500/30 rounded-3xl p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <h2 className="text-3xl font-black text-orange-400">Skill Filter</h2>

          <button onClick={() => setSkillModal(false)}className="bg-orange-500 px-4 py-2 rounded-xl font-bold">X</button>
        </div>

        <div className="flex flex-wrap gap-3 mb-8">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveSkill(cat)} className={`px-5 py-2 rounded-full font-bold ${
                activeSkill === cat? "bg-orange-500 text-white": "bg-white text-black"
              }`}
            >{cat}</button>
          ))}
        </div>
      ...
      {projects.map((project) => (
        <div key={project.title} className="group bg-[#151515] border border-white/10 rounded-3xl p-6 hover:border-orange-400 hover:-translate-y-2 transition">
          ...
          {project.detailsLink ? (
            ...
            <button className="w-full bg-gray-700 py-3 rounded-xl font-bold cursor-not-allowed">Coming Soon</button>)}
        </div>
      ))}
    </div>
  </section>
  ...
  <section id="contact" className="py-20 px-6 bg-black/30">
    <div className="max-w-4xl mx-auto">
      <h2 className="text-4xl font-black text-center text-orange-400 mb-4">Contact Me</h2>

      <p className="text-center text-gray-400 mb-10">Send your message using the box below.</p>
      ...
      <form onSubmit={handleSubmit} className="bg-[#111] border border-orange-500/20 rounded-3xl p-6 md:p-8">
        ...
        <button className="w-full bg-orange-500 py-4 rounded-xl font-bold hover:bg-orange-400">Send Message</button>
      </form>
    </div>
  </section>
